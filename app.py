    ...
    try:
        ...
        for row in all_rows:
            try:
                ...
                # --- 6. CS / VS KONTROLÜ (GÜNCELLENMİŞ) ---
                raw_val = 0
                
                # Önce ne varsa çek (CS veya Vision Score olabilir)
                # 'minions' class'ı Support oynanınca Vision Score gösteriyor olabilir.
                cs_div = row.find("div", class_="minions")
                if cs_div:
                    m = re.search(r"(\d+)", cs_div.text)
                    if m: raw_val = int(m.group(1))
                else:
                    # Div yoksa metin içinde ara
                    m = re.search(r"(\d+)\s*CS", row.text, re.IGNORECASE)
                    if m: raw_val = int(m.group(1))

                # Varsayılan: Bulduğumuz sayıyı CS olarak kabul et
                display_stat = f"{raw_val} CS"

                # KARAR MEKANİZMASI:
                # Oyun 10 dk'dan uzunsa VE okunan sayı 70'ten küçükse -> Bu bir Görüş Skorudur.
                # (Çünkü 15. dakikada 15 CS mantıksızdır, o 15 Vision Score'dur)
                if game_duration > 10 and raw_val < 70:
                    display_stat = f"GÖRÜŞ {raw_val} VS"
                else:
                    # Değilse normal CS'dir (150 CS gibi)
                    display_stat = f"{raw_val} CS"
