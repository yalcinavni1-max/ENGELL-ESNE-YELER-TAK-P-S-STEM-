    ...
    try:
        ...
        for row in all_rows:
            try:
                ...
                # --- 6. CS / VISION SCORE (DOĞRUDAN ÇEKME) ---
                # League of Graphs, Support oynanmışsa 'minions' div'inin içine CS yerine Vision Score yazar.
                # Yanında da 'Vision Score' veya 'Görüş Skoru' yazısı (veya ikonu) olur.
                
                raw_stat_text = ""
                cs_div = row.find("div", class_="minions")
                
                if cs_div:
                    raw_stat_text = cs_div.text.strip() # Örn: "145 CS" veya "35 Vision"
                
                # Varsayılan değer
                display_stat = raw_stat_text 
                ...
                # AKILLI KONTROL:
                # Eğer oyun 10dk'dan uzunsa VE sayı 70'ten küçükse -> Bu kesinlikle Vision Score'dur (veya Support oyunudur)
                if game_duration > 10 and val_num < 70:
                    # League of Graphs bazen "Vision" yazmaz, sadece sayı yazar.
                    # Biz bunu netleştirmek için "VS" ekleyelim.
                    display_stat = f"GÖRÜŞ {val_num} VS"
                else:
                    # Değilse normal CS'dir
